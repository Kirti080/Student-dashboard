...
export function PageHeader({ title, subtitle }: PageHeaderProps) {
  ...
  return <header className="mb-6 flex items-center justify-between gap-3 sm:mb-8">
    <div className="flex min-w-0 items-center gap-3"><SidebarTrigger className="rounded-xl bg-white/80 p-2 text-slate-600 shadow-sm dark:bg-slate-800 dark:text-slate-300" /><div className="min-w-0"><h1 className="truncate text-xl font-black tracking-tight text-slate-900 dark:text-white sm:text-2xl">{title}</h1><p className="truncate text-xs text-slate-500 dark:text-slate-400 sm:text-sm">{subtitle}</p></div></div>
